...
function suggestHintByMessage(message) {
  ...
  if (text.includes("already exists")) {
    return "资源 ID 或名称与后端冲突；更换统一前缀后重试。";
  }
  if (text.includes("unknown") || text.includes("outside agent closure")) {
    return "检查 agent/route/task 引用闭包，确保只引用后端已有或本次 bundle 新建资源。";
  }
  if (text.includes("missing required business contract fields")) {
    return "补齐 TaskStep 业务约束：目标、输入、输出、限制、边界、异常处理、验收标准。";
  }
  if (text.includes("prompt includes forbidden framework runtime terms")) {
    return "角色 prompt 只保留职能/工具/方法，删除框架运行术语与路由协议描述。";
  }
  if (text.includes("artifact must be workspace-relative")) {
    return "artifact 改为 workspace/ 相对路径，并与 write_set 对齐。";
  }
  if (text.includes("write_set")) {
    return "为每个任务定义明确 write_set，覆盖本任务 artifact，且避免跨任务重叠。";
  }
  if (text.includes("auto_start")) {
    return "workflow_run.auto_start 必须为 false，由主控显式启动 run。";
  }
  return "根据校验错误修正 bundle，再次执行 check。";
}
...
function buildHints(errors, warnings) {
  ...
  const warningHint =
    Array.isArray(warnings) && warnings.length > 0
      ? ["存在质量告警；建议先修告警项再执行 publish 以提高真实运行收敛率。"]
      : [];
