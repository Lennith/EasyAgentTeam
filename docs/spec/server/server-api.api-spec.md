...
- `GET /api/settings` 返回 `providers` 作为唯一运行时 settings 模型：
...
- retry-dispatch 审计事件按 `SESSION_RETRY_DISPATCH_REQUESTED`、`SESSION_RETRY_DISPATCH_ACCEPTED`、`SESSION_RETRY_DISPATCH_REJECTED` 区分
