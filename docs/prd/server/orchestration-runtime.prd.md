...
- project / workflow dispatch lease 是活跃调度并发上限与恢复收口的权威状态；dispatch launch 前必须在同一 lease-file mutex 内完成 active count 检查与 open lease 写入，`maxConcurrentDispatches` 按未过期的 `open|closing|retrying` durable lease 执行，手动 `force` 只绕过目标可用性 gate，不绕过全局活跃 dispatch 上限
- 进程内 single-flight 只作为同一进程内的 launch gate；它不能代表 provider 仍在运行，也不能作为重启后的调度权威
- dispatch started 阶段必须在 runner try/catch 内执行；如果 lease 打开后 started event 写入失败，必须走失败 lifecycle 并关闭对应 lease
