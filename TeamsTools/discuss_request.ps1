...
if (-not $message.Trim()) { Write-ToolError 'LOCAL_MESSAGE_REQUIRED' 'message cannot be empty.' 'Pass -message with concrete question/request.' }
if ($round -le 0) { $round = 1 }
$resolvedThreadId = if ($thread_id.Trim()) { $thread_id.Trim() } else { "$resolvedTaskId-$(Get-Date -Format 'yyyyMMddHHmmss')" }
...
$body = @{
  ...
  discuss = @{ thread_id = $resolvedThreadId; round = $round }
}
...
if ($raw) {
  ...
  $msg = if ($raw.error -and $raw.error.message) { [string]$raw.error.message } elseif ($raw.message) { [string]$raw.message } else { 'Discuss request rejected by backend.' }
  $nextAction = if ($raw.next_action) { [string]$raw.next_action } elseif ($raw.hint) { [string]$raw.hint } else { 'Check route target, task_id, thread_id, and round then retry once.' }
