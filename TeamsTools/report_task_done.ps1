param(
  [Parameter(Mandatory=$true)][string]$task_report,
  [string]$task_id = '',
  [string]$manager_url = '',
  [string]$project_id = ''
)
...
function Resolve-NextAction([string]$Code) {
  switch ($Code) {
    'TASK_PROGRESS_REQUIRED' { return 'Update Agents/<role>/progress.md and include this task_id, then retry report_task_done.' }
    'TASK_RESULT_INVALID_TARGET' { return 'Verify task ownership/session and report only your assigned task.' }
    'TASK_REPORT_NO_STATE_CHANGE' { return 'Do not resend same report. Add new evidence or report a different unresolved task.' }
...
if (-not $task_report.Trim()) { Write-ToolError 'LOCAL_REPORT_FILE_REQUIRED' 'task_report path is required.' 'Pass -task_report with report file path.' }
if (-not (Test-Path -LiteralPath $task_report)) { Write-ToolError 'LOCAL_REPORT_FILE_NOT_FOUND' "task_report not found: $task_report" 'Generate report file first, then retry.' }

$reportContent = Get-Content -LiteralPath $task_report -Raw -Encoding UTF8
if (-not $reportContent.Trim()) { Write-ToolError 'LOCAL_REPORT_FILE_EMPTY' 'task_report file is empty.' 'Write concrete evidence and conclusion into report file.' }
