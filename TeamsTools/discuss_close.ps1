...
if (-not $message.Trim()) { Write-ToolError 'LOCAL_MESSAGE_REQUIRED' 'message cannot be empty.' 'Pass -message with final close summary.' }
if (-not $thread_id.Trim()) { Write-ToolError 'LOCAL_THREAD_REQUIRED' 'thread_id is required for discuss close.' 'Pass the original discuss thread_id.' }
...
if ($raw) {
  ...
  $msg = if ($raw.error -and $raw.error.message) { [string]$raw.error.message } elseif ($raw.message) { [string]$raw.message } else { 'Discuss close rejected by backend.' }
  $nextAction = if ($raw.next_action) { [string]$raw.next_action } else { 'Check route target, thread_id, and round then retry once.' }
