# 碎碎念
- 一句话功能，就是可以自己配置Agent，然后组成一个Team，让他们干活，具体怎么组织看你Prompt。Agent会根据相互角色来相互指派任务，沟通和推进任务，直到完成，直到完成，直到完成。
- 允许的自动派发轮数在项目设置里。 目前没有用Token计算，是因为我的coding plan是用对话轮数来算额度的。
...
- 真的不是打广告，但是MiniMax这个3小时500次prompt的额度真实大大加快了测试进程。
- 项目下有个端对端测试，会注入2个测试项目并自动开工，可以看这个东西是如何工作的。
