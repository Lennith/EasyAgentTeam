...
import type { ProjectDetail, SessionRecord, TaskTreeNode, TaskState, LockRecord, EventRecord, AgentIOTimelineItem } from "@/types";
...
export function TaskboardView({ projectId, sessions, tasks, reload }: TaskboardViewProps) {
  ...
  return (
    <section style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {dispatchError && <div className="error-message" style={{ flexShrink: 0 }}>{dispatchError}</div>}
      {dispatchSuccess && <div className="success-message" style={{ flexShrink: 0 }}>{dispatchSuccess}</div>}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "16px", flex: 1, minHeight: 0, overflow: "auto", paddingRight: "4px" }}>
        {Object.entries(groupedTasks).map(([groupName, groupTasks]) => (
          <div key={groupName} className="card" style={{ flexShrink: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
  ...
  return (
    <div style={{ 
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "6px" }}>
        ...
        <span style={{ fontSize: "11px", color: stateColors[task.state] }}>
