...
function assertDependenciesValid(
  ...
  const findPathToAncestor = (startDependencyTaskId: string): string[] | null => {
    const path: string[] = [];
    ...
    const dfs = (node: string): boolean => {
      ...
      if (visiting.has(node) || explored.has(node)) {
        path.pop();
        return false;
      }
      visiting.add(node);
      const nextDependencies = depsByTask.get(node) ?? [];
      for (const nextDependencyTaskId of nextDependencies) {
        if (dfs(nextDependencyTaskId)) {
          return true;
        }
      }
    ...
    if (!dfs(startDependencyTaskId)) {
      return null;
    }
    return [...path];
  };

  const transitiveForbiddenDependencies = dependencies
    .map((dependencyTaskId) => {
      const dependencyChain = findPathToAncestor(dependencyTaskId);
      if (!dependencyChain) {
        return null;
      }
      return {
        dependency_task_id: dependencyTaskId,
        hit_ancestor_task_id: dependencyChain[dependencyChain.length - 1],
  ...
  if (transitiveForbiddenDependencies.length > 0) {
    throw new TaskboardStoreError(
      `dependencies cannot transitively reach parent/ancestor tasks: ${transitiveForbiddenDependencies
        .map((entry) => `${entry.dependency_task_id}->${entry.hit_ancestor_task_id}`)
        .join(", ")}`,
      "TASK_DEPENDENCY_ANCESTOR_FORBIDDEN",
      {
        task_id: taskId,
        parent_task_id: parentTaskId,
        ancestor_task_ids: ancestorTaskIds,
        transitive_forbidden_dependencies: transitiveForbiddenDependencies
      }
    );
  }
