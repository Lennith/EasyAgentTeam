...
import { OrchestratorKernel } from "./kernel/orchestrator-kernel.js";
import { OrchestratorSingleFlightGate } from "./kernel/single-flight.js";
...
export function createWorkflowOrchestratorComposition(
  ...
  const clearScopedState = (runId: string) => {
    clearRunScopedState(
      runId,
      runHoldState,
  ...
  const clearAllTransientState = () => {
    runHoldState.clear();
    runAutoFinishStableTicks.clear();
    sessionHeartbeatThrottle.clear();
    inFlightDispatchSessionKeys.clear();
  };
  const buildRunSessionKey = (runId: string, sessionId: string) => buildOrchestratorContextSessionKey(runId, sessionId);
  ...
  const tickService = new WorkflowTickService({
    ...
    activeRunIds,
    runHoldState,
    pruneInactiveRunScopedState: (activeIds) =>
  ...
  const runLifecycleService = new WorkflowRunLifecycleService({
    ...
    providerRegistry: input.providerRegistry,
    activeRunIds,
    clearRunScopedState: (runId) => clearScopedState(runId),
