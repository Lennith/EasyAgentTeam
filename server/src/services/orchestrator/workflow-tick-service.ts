...
export class WorkflowTickService {
  ...
  async tickLoop(): Promise<void> {
    ...
    await this.options.kernel.runTick({
      ...
      tickContext: async (run) => {
        ...
        await this.handleRunHoldState(run);
        await this.tickPipeline.run({ run, runtime, sessions });
      }
    });
  }
  ...
  private async handleRunHoldState(run: WorkflowRunRecord): Promise<void> {
    ...
    const previousHold = this.options.runHoldState.get(run.runId);
    if (previousHold === undefined || previousHold !== holdEnabled) {
      ...
      this.options.runHoldState.set(run.runId, holdEnabled);
    }
  }
}
