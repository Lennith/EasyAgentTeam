...
export interface WorkflowSessionTimeoutEvidenceInput {
  session: Pick<
    WorkflowSessionRecord,
    "sessionId" | "status" | "currentTaskId" | "lastActiveAt" | "updatedAt" | "createdAt"
  >;
  nowMs: number;
  timeoutMs: number;
  events: WorkflowRunEventRecord[];
  task: Pick<WorkflowTaskRuntimeRecord, "taskId" | "state"> | null;
}

export interface WorkflowSessionTimeoutEvidenceDecision {
...
function findRecentTerminalReportEvent(input: WorkflowSessionTimeoutEvidenceInput): WorkflowRunEventRecord | null {
...
export function resolveWorkflowSessionTimeoutEvidence(
  input: WorkflowSessionTimeoutEvidenceInput
): WorkflowSessionTimeoutEvidenceDecision {
