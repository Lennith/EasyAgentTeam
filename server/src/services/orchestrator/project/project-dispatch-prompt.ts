...
export function buildProjectDispatchPrompt(context: ProjectDispatchPromptContext): string {
  ...
  lines.push(...buildProviderDispatchContractLines(session.provider).map((line) => `- ${line}`));
  lines.push("- focus task context:");
  lines.push(`  - title: ${focusTask?.title ?? "(none)"}`);
  lines.push(...buildTaskSubtreeContractLines().map((line) => `- ${line}`));
  lines.push(...buildFocusTaskExecutionContractLines().map((line) => `- ${line}`));
  return lines.join("\n");
}
