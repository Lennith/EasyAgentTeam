...
import { findLatestOpenDispatch, readPayloadString } from "./dispatch-engine.js";
import {
...
export async function markProjectTimedOutSessions(
  ...
  for (const session of sessions) {
    ...
    if (!Number.isFinite(lastActive) || now - lastActive < dependencies.sessionRunningTimeoutMs) {
      continue;
    }
