...
export async function buildAgentIOTimeline(
  ...
  const items: AgentIOTimelineItem[] = [];
  for (const event of events) {
    ...
    if (event.eventType === "MESSAGE_ROUTED") {
      ...
      const isDiscuss = Boolean(messageType && messageType.startsWith("TASK_DISCUSS"));
      items.push({
        itemId: event.eventId,
        kind: isDiscuss ? "task_discuss" : "message_routed",
        createdAt: event.createdAt,
        from: computeSenderFrom(payload),
        toRole: (payload.toRole as string | null | undefined) ?? null,
    ...
    if (event.eventType === "TASK_ACTION_RECEIVED") {
      ...
      const toRole = (payload.toRole as string | null | undefined) ?? (actionType === "TASK_REPORT" ? "manager" : null);
      items.push({
        itemId: event.eventId,
        kind: "task_action",
        createdAt: event.createdAt,
    ...
    if (event.eventType === "TASK_ACTION_REJECTED") {
      const actionType = (payload.actionType as string | null | undefined) ?? null;
      const code = readPayloadString(payload, "error_code") ?? "TASK_ACTION_REJECTED";
  ...
  const limit = options.limit && Number.isFinite(options.limit) ? Math.max(1, Math.floor(options.limit)) : undefined;
